...
void DX12UploadBuffer::Initialize(ID3D12Device* device, uint64_t size)
{
...
D3D12_GPU_VIRTUAL_ADDRESS DX12UploadBuffer::UploadCBVSRVUAV(void* data, UINT64 size)
{
	...
	memcpy(m_mappedAddress + m_usedSize, data, size);

	m_usedSize += (size + ConstBufferSize - 1) & ~(ConstBufferSize - 1);
	//m_buffersOffset = m_usedSize / ConstBufferSize;
