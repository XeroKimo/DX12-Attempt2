...
namespace RendererDX12
{
    ...
    using namespace LevelTwo;
        DX12DeviceCommandModule::DX12DeviceCommandModule(BaseDevice* device, DX12ManagerCommandAllocator* manager, UINT directQueues, UINT computeQueues, UINT copyQueues)
        {
            m_device = device;
        ...
        void DX12DeviceCommandModule::StallQueue(D3D12_COMMAND_LIST_TYPE stallType, UINT stallIndex, Fence* fence, UINT64 fenceValue)
        {
            DX12CommandQueue* stalledQueue = GetCommandQueue(stallType, stallIndex);
            if (stalledQueue)
        ...
        void DX12DeviceCommandModule::StallQueue(DX12CommandQueue* queueToStall, Fence* fenceToWait, UINT64 valueToWait)
        {
