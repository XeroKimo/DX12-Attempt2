...
void DX12Device::Initialize(DX12ManagerCommandAllocator* manager, D3D_FEATURE_LEVEL featureLevel, UINT adapterID, UINT directQueues, UINT copyQueues, UINT computeQueues)
{
    m_device = make_unique<DX12BaseDevice>();
...
void DX12Device::ExecuteCommandLists(UINT numCommandLists, ID3D12CommandList* const* commandLists, UINT queueIndex)
{
    ...
    case D3D12_COMMAND_LIST_TYPE_DIRECT:
        if (m_directQueue.size() <= queueIndex - 1)
            m_directQueue[queueIndex]->GetBaseCommandQueue()->GetCommandQueue()->ExecuteCommandLists(numCommandLists, commandLists);
        break;
    case D3D12_COMMAND_LIST_TYPE_COMPUTE:
        if (m_computeQueue.size() <= queueIndex - 1)
            m_computeQueue[queueIndex]->GetBaseCommandQueue()->GetCommandQueue()->ExecuteCommandLists(numCommandLists, commandLists);
