...
#include "Level 2/DX12ManagerCommandList.h"
...
void DX12ManagerCommandList::Initialize(ID3D12Device* device, D3D12_COMMAND_LIST_TYPE type, std::vector<unique_ptr<DX12CommandQueue>>* commandQueues, DX12ManagerCommandAllocator* allocatorManager)
{
	...
	m_waitingAllocators.resize(commandQueues->size());
}
...
void DX12ManagerCommandList::ExecuteList(UINT queueIndex)
{
	...
	{
		commandLists.push_back(listToExecute[v]->GetInterface());
		m_inactiveLists.push_back(listToExecute[v]);
	}

    (*m_pCommandQueues)[queueIndex]->GetBase()->GetInterface()->ExecuteCommandLists(static_cast<UINT>(commandLists.size()), commandLists.data());
...
{
    ID3D12CommandList* list[] = { commandList->GetInterface() };
    (*m_pCommandQueues)[commandList->GetQueuePreference()]->GetBase()->GetInterface()->ExecuteCommandLists(1, list);
    (*m_pCommandQueues)[commandList->GetQueuePreference()]->SetActiveAllocator(commandList->GetCommandAllocator());
}

void DX12ManagerCommandList::ReEnlistInactiveCommandList(shared_ptr<DX12CommandList>& commandList)
{
