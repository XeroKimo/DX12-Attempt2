...
{
	m_device.Initialize(D3D_FEATURE_LEVEL_11_0, 0, &m_allocatorManager,1,0,0);
	//m_swapChain.Initialize(m_device.GetBase()->GetInterface(), m_device.GetBase()->GetNodeMask(), m_device.GetCommandQueue(D3D12_COMMAND_LIST_TYPE_DIRECT, 0)->GetBase()->GetInterface(), windowHandle, windowWidth, windowHeight);
